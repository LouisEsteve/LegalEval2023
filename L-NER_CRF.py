...
def generate_features_from_str(
		input_str	:	str
	):
	'''
	This function is to be called if no features were stored previously.
	...
	if "drop_rows_if_true" in config:
		...
		for j in X_features:
			...
			for i in config["drop_rows_if_true"]:
				...
				if i in j and j[i] == True:
					appending	=	False
					break
			if appending:
				new_X_features.append(j)
			
		# print(f'Dropped {len_df-len(features_df)}')
		# print(f'Dropped {len_X_features-len(X_features)}')
...
def main() -> int:
		...
		if sys.argv[i] == "-f":
			...
			if sys.argv[i].endswith('.json'):
				try:
					...
					# corpus_content["_FILE_PATH_"]	=	sys.argv[i]
					# corpus_objects.append(corpus_content)
			...
			for j in ld:
				...
				try:
					...
					corpus_content			=	json.load(file)
					# corpus_content["_FILE_PATH_"]	=	local_path
					# corpus_objects.append(corpus_content)
	...
	for i in corpus_objects:
		# if "annotations" not in corpus_objects[i]:
			# corpus_objects[i]["annotations"]	=	[]
		# if "annotations" not in corpus_objects[i][0]:
			# corpus_objects[i][0]["annotations"]	=	[]
		corpus_objects[i][0]["annotations"]	=	[]
		...
		for j in corpus_objects[i]:
			print(j["id"],end="\r")
			text	=	j["data"]["text"]
			# X_features	=	generate_features_from_str(text)
			...
			for k in range(len(prediction)):
				if prediction[k].startswith('B-') or prediction[k] == 'O':
					if start_index != None:
						value_obj	=	{
							"start"		:	start_index,
							"end"		:	end_index,
						...
						result_obj["result"].append({
							"value"		:	value_obj,
							# "id"		:	str(hash(value_obj)),
							# "id"		:	str(hash(corpus_objects[i][j]["id"] + value_obj["text"] + str(value_obj["start"]) + str(value_obj["end"]))),
						...
						NE_type		=	None
						annot_count	+=	1
				if prediction[k].startswith('B-'):
					start_index	=	X_offsets[k][0]
					end_index	=	X_offsets[k][1]
					NE_type		=	prediction[k][2:]
				
				elif prediction[k].startswith('I-'):
					end_index	=	X_offsets[k][1]
				# previous_IOB	=	prediction[k][0]
		...
		# corpus_objects[i]["annotations"].append(result_obj)
		corpus_objects[i][0]["annotations"].append(result_obj)
		...
		try:
			...
			print(e)
			print(f"Could not output to file {output_path}. See message above.")
